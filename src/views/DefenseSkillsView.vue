...
const getData = async () => {
    ...
    console.log('data', defenseSkills.value);
};
...
const delSkill = async (item: Skill) => {
    ...
    if (confirmDelete) {
        ...
        console.log(data);
        try {
            const result = await axios.delete('http://localhost:3000/delDefenseSkill', { data });
...
let showData = ref<Skill>();
const showClick = (_showData: Skill) => {
...
let putData = ref<Skill>();
const putClick = (_putData: Skill) => {
